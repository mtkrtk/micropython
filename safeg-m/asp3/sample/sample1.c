 ...
 *  の責任を負わない．
 * 
 *  $Id: sample1.c 7 2020-03-06 01:31:28Z ertl-komori $
 */
...
#include "core_insn.h"
void __attribute__((cmse_nonsecure_entry)) nonsecure_task(int n)
{
	int_t		tskno = (int_t)4;
	char		c;

	syslog(LOG_NOTICE, "NS task is running (%03d).       #", n);
	c = message[tskno-1];
	message[tskno-1] = 0;
	switch (c) {
	case 'e':
...
void __attribute__((cmse_nonsecure_entry)) nonsecure_message(const char *str)
{
